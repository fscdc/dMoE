...
class LLaDA2MoeGate(nn.Module):
    ...
    def _select_coreset_vote_blocks(self, block_scores: torch.Tensor):
        # valid token mask: padded tokens are all -inf in routing scores
        valid_token_mask = torch.isfinite(block_scores).any(dim=-1)  # [bsz, n_blocks, block_len]

        # token-level local top-k
        ...
        # remove padded token contribution
        masked_scores = masked_scores.masked_fill(~valid_token_mask.unsqueeze(-1), 0.0)

        # aggregate to block-level expert scores
        votes = masked_scores.sum(dim=2)  # [bsz, n_blocks, num_experts]

        # normalize within each block
        votes = votes / (votes.sum(dim=-1, keepdim=True) + 1e-20)

        # dynamic coreset selection by top-p cumulative probability
        sorted_votes, sorted_idx = torch.sort(votes, dim=-1, descending=True)   # [bsz, n_blocks, num_experts]
        cum_probs = torch.cumsum(sorted_votes, dim=-1)                          # [bsz, n_blocks, num_experts]
        ...
        # safeguard: at least keep guard_size experts for each block
        num_selected = coreset_mask.sum(dim=-1)  # [bsz, n_blocks]
        guard_size = 12
        need_fix = num_selected < guard_size

        if need_fix.any():
            fallback_idx = torch.topk(votes, k=guard_size, dim=-1).indices  # [bsz, n_blocks, guard_size]
    ...
    def forward(self, hidden_states):
        block_size = 32  # @sicheng: for code simplicity, we fix block size for training here.

        bsz, seq_len, _ = hidden_states.shape
        # print(f"[sicheng] bsz={bsz}, seq_len={seq_len}, block_size={block_size}") # test
        ...
        if self.config.mode is None:
            raise ValueError("coreset setting is wrong!")

        block_size = min(block_size, seq_len)

        # Parallel block processing over whole sequence
        pad_len = (block_size - (seq_len % block_size)) % block_size
        if pad_len > 0:
            pad_scores = torch.zeros(
                (bsz, pad_len, self.num_experts),
                dtype=scores.dtype,
                device=scores.device,
            )
            pad_scores_for_routing = torch.full(
                (bsz, pad_len, self.num_experts),
                float("-inf"),
                dtype=scores_for_routing.dtype,
                device=scores_for_routing.device,
            )
            scores_padded = torch.cat([scores, pad_scores], dim=1)
            scores_for_routing_padded = torch.cat(
                [scores_for_routing, pad_scores_for_routing], dim=1
            )
        else:
            scores_padded = scores
            scores_for_routing_padded = scores_for_routing

        padded_seq_len = scores_padded.shape[1]
        n_blocks = padded_seq_len // block_size

        # [bsz, n_blocks, block_size, E]
        scores_blocks = scores_padded.view(bsz, n_blocks, block_size, self.num_experts)
        routing_blocks = scores_for_routing_padded.view(
            bsz, n_blocks, block_size, self.num_experts
        )

        # select coreset for every block in parallel
        if self.config.mode == "aggregate":
            coreset_mask = self._select_coreset_vote_blocks(routing_blocks)  # [bsz, n_blocks, E]
        else:
            raise ValueError("unsupported mode")

        # broadcast same coreset to all tokens inside each block
        token_coreset_mask = coreset_mask.unsqueeze(2).expand(
            -1, -1, block_size, -1
        )  # [bsz, n_blocks, block_size, E]

        constrained_scores = routing_blocks.masked_fill(
            ~token_coreset_mask, float("-inf")
        )

        # do routing for all tokens in all blocks together
        _, full_topk_idx = self.group_limited_topk(
            constrained_scores.reshape(-1, self.num_experts)
        )
        full_topk_idx = full_topk_idx.view(bsz, padded_seq_len, self.top_k)

        # remove padded positions
        if pad_len > 0:
            full_topk_idx = full_topk_idx[:, :seq_len, :]

        selected_scores = torch.gather(scores, dim=-1, index=full_topk_idx).view(
            -1, self.top_k
        ).type_as(logits)
...
class LLaDA2MoeModelLM(LLaDA2MoePreTrainedModel, GenerationMixin):
    ...
    def generate(
                ...
                if eos_early_stop and (x0[transfer_index] == eos_id).any():
                    ...
                    if len(eos_pos_in_x[0]) > 0:
                        ...
                        if (cur_x[0, prompt_length:eos_pos] != mask_id).all():
                            ...
                            print(f"[sicheng] Unique experts per layer (early stop): {avg_unique_experts_per_layer}")
        ...
        print(f"[sicheng] Unique experts per layer (no early stop): {avg_unique_experts_per_layer}")

        return generated_answer[:, input_ids.shape[1] : input_ids.shape[1] + first_mask_position + 1]


    # @sicheng: for calculating expert count.
    @staticmethod
