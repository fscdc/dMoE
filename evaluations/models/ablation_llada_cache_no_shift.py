    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, past_q_pos: Optional[torch.bool] = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            if past_q_pos:
                ...
                if pos_sin.shape[0] < q_.shape[0]:
                    assert pos_sin.shape[0] * 2 == q.shape[0]
                    pos_sin = pos_sin.repeat(2, 1, 1, 1)
                    pos_cos = pos_cos.repeat(2, 1, 1, 1)
                ...
                if pos_sin.shape[0] < k_.shape[0]:
                    assert pos_sin.shape[0] * 2 == k.shape[0]
                    pos_sin = pos_sin.repeat(2, 1, 1, 1)
                    pos_cos = pos_cos.repeat(2, 1, 1, 1)
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att = self._activation_checkpoint_fn(  # type: ignore
                ...
                layer_past=layer_past, use_cache=use_cache, transfer_idx=transfer_idx
            )
        else:
            att = self.attention(
                q, k, v, 
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                x, cache = self._activation_checkpoint_fn(  # type: ignore
                    block, x, attention_mask=attention_mask, attention_bias=attention_bias, 
                    layer_past=layer_past, use_cache=use_cache, block_idx=block_idx
                )
            else:
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(
