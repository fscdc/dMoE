...
class LLaDA2MoeSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: LLaDA2MoeConfig):
        ...
        # # @sicheng: for weights analysis
        # self.force_dense_eval = False
        # self.analysis_token_positions = None   # flattened token positions, shape [N]
        # self.analysis_expert_ids = None        # expert ids paired with token positions, shape [N]

        # self.last_topk_idx = None              # [bsz, seq_len, top_k]
        # self.last_topk_weight = None           # [bsz, seq_len, top_k]
        # self.last_router_logits = None         # [bsz, seq_len, num_experts]
    ...
    # # @sicheng: two helper function for weights analysis
    # def set_analysis_ablation(self, token_positions, expert_ids):
    #     if token_positions is None or expert_ids is None:
    #         self.analysis_token_positions = None
    #         self.analysis_expert_ids = None
    #         return

    #     if not torch.is_tensor(token_positions):
    #         token_positions = torch.tensor(token_positions, dtype=torch.long)
    #     if not torch.is_tensor(expert_ids):
    #         expert_ids = torch.tensor(expert_ids, dtype=torch.long)

    #     self.analysis_token_positions = token_positions
    #     self.analysis_expert_ids = expert_ids
    # def clear_analysis_ablation(self):
    #     self.analysis_token_positions = None
    #     self.analysis_expert_ids = None
    ...
    # # @sicheng: forward with weights analysis
    # def forward(self, hidden_states):
    #     identity = hidden_states
    #     bsz, seq_len, h = hidden_states.shape
    #     profiler = self.profiler
    ...
    #     def _dense_moe_forward(hidden_states, topk_idx, topk_weight):
    #         hidden_states_flat = hidden_states.view(-1, hidden_states.shape[-1])   # [num_tokens, h]
    #         num_tokens = hidden_states_flat.shape[0]

    #         repeated_hidden = hidden_states_flat.repeat_interleave(
    #             self.num_experts_per_tok, dim=0
    #         )  # [num_tokens * top_k, h]

    #         flat_topk_idx = topk_idx.view(-1)  # [num_tokens * top_k]
    #         expert_out = torch.empty_like(repeated_hidden)

    #         for i, expert in enumerate(self.experts):
    #             mask = (flat_topk_idx == i)
    #             if mask.any():
    #                 expert_out[mask] = expert(repeated_hidden[mask])

    #         # [num_tokens, top_k, h]
    #         expert_out = expert_out.view(num_tokens, self.num_experts_per_tok, h)

    #         # ===== analysis ablation: zero out selected (token, expert) contribution =====
    #         if self.analysis_token_positions is not None and self.analysis_expert_ids is not None:
    #             token_positions = self.analysis_token_positions.to(topk_idx.device)
    #             expert_ids = self.analysis_expert_ids.to(topk_idx.device)

    #             for pos, exp_id in zip(token_positions.tolist(), expert_ids.tolist()):
    #                 if 0 <= pos < num_tokens:
    #                     slot_mask = (topk_idx[pos] == exp_id)   # [top_k]
    #                     if slot_mask.any():
    #                         expert_out[pos, slot_mask, :] = 0.0

    #         y = (expert_out * topk_weight.unsqueeze(-1)).sum(dim=1)  # [num_tokens, h]
    #         y = y.to(hidden_states_flat.dtype).view(bsz, seq_len, h)
    #         return y
    ...
    #             # save last routing info for analysis
    #             self.last_topk_idx = topk_idx.view(bsz, seq_len, -1).detach()
    #             self.last_topk_weight = topk_weight.view(bsz, seq_len, -1).detach()
    #             self.last_router_logits = router_logits.view(bsz, seq_len, -1).detach()

    #             if self.training or self.force_dense_eval or (self.analysis_token_positions is not None):
    #                 y = _dense_moe_forward(hidden_states, topk_idx, topk_weight)
    #             else:
    #                 hidden_states_flat = hidden_states.view(-1, hidden_states.shape[-1])
    #                 with profiler.time(f"{prefix}.moe_infer"):
    #                     y = self.moe_infer(hidden_states_flat, topk_idx, topk_weight).view(
    #                         bsz, seq_len, h
    #                     )
